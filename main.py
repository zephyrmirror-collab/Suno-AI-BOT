...
# Мы склеиваем сессию в формат, который требует библиотека suno
...
try:
    ...
    logger.error(f"Critical error during Suno init: {e}")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.message.reply_text("🎤 Отправь текст или описание:")
...
async def onMessage(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        is_custom = (chat_states[chat_id].get('mode') == 'custom')
